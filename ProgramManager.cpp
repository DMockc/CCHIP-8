	...
	case DRW_VX_VY_N:
		...
		for (int row = 0; row < instruction.FourthQuarter; row++)
		{
			...
			for (int column = 0; column < BITS_IN_A_BYTE; column++)
			{
				...
				if (DisplayMatrix::get(y_selectedCoord, x_selectedCoord) == 1 && color == 1)
				{
					m_cpu.V[0xF] = (DisplayMatrix::get(y_selectedCoord, x_selectedCoord) == 1 && color == 1); //Update collision
				}

				DisplayMatrix::set(
					y_selectedCoord, x_selectedCoord,
					DisplayMatrix::get(y_selectedCoord, x_selectedCoord) ^ color
				); //Make XOR in [x, y]
				...
				m_display.coords_buffer.push_back(
					DRW_COORD(
						x_selectedCoord,
						y_selectedCoord,
						DisplayMatrix::get(y_selectedCoord, x_selectedCoord)
					)
				);
